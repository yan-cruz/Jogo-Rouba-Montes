...
// Função principal - Início
    int main (){
        // Setando o jogo em português, para o uso de acentos e caracteres 
        ...
        while (1) {
            ...
            if (scanf("%d", &quantJogadores) != 1 || quantJogadores < 2) {
                printf("\n  Entrada inválida. Insira um número inteiro maior ou igual a 2.\n");
                // Limpa o buffer do scanf para evitar loop infinito caso o usuário insira letras
                while (getchar() != '\n');
            } else {
                break;
            }
        ...
        tJogador *players;
        players = (tJogador*)malloc(quantJogadores * sizeof(tJogador));
        if (players == NULL){
        ...
        while (1) {
            ...
            if (scanf("%d", &quantBaralhos) != 1 || quantBaralhos < 1) {
                printf("\n  Entrada inválida. Insira um número inteiro maior ou igual a 1.\n");
                // Limpa o buffer do scanf para evitar loop infinito caso o usuário insira letras
                while (getchar() != '\n');
            } else {
                break;
        ...
        tCarta *cartaVez;
        cartaVez = (tCarta*)malloc(sizeof(tCarta));
        if (cartaVez == NULL) {
...
// Corpo das funções - Início
    ...
    tPilha* criarBaralhos(int numBaralhos) {
        ...
        tCarta *baralhos;
        baralhos = (tCarta*)malloc(tamanhoTotal * sizeof(tCarta));
    ...
    int ganhadores(tJogador *jogadores, int quantJogadores, tLista *descarte){
        ...
        int *ganhadores;
        ganhadores = (int*)malloc(quantJogadores * sizeof(int));
        if (ganhadores == NULL) {
            printf("\n\n  Erro ao alocar memória para o jogo.");
            return 0;
        }
        ...
        // Imprime os vencedores e suas pilhas
        if (cont > 1){
            printf("\n\n  Os vencedores são:");
            for (int i = 0; i < quantJogadores; i++){
        ...
        // Zera a variável cont, para a correta impressão dos resultados
        if (cont == 1){
            cont = 0;
        }
    ...
    int comparaCartaVez(tCarta *cartaVez, tLista *listaDescarte, tJogador *jogadores, int quantJogadores, int jogadorAtual) {
        ...
        // A carta da vez não combina com nenhuma carta na área de descarte ou no topo dos montes dos outros jogadores
        return 0;
    };
